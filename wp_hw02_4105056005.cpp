		...
		//印出房價
			...
			*pfront = 1;
			...
			for (*pi = 0; *pi < 9; (*pi)++) {
				...
				Console::Write("|{0,7}元", plocaMon[*pfront]);
				(*pfront)++;
			}
			Console::Write("|   $$$$$$$   | ");
			(*pfront)++;
		...
		for (*pj = 0; *pj < 4; (*pj)++) {
			...
			if (*pj == 0) {
				if (phouses[*pback] > 0 && *pi == 0)
					Console::Write("|    {0}'s house|       □:1th player        ■:2th player    ", owner[*pback]);
				else if(*pi == 0)
				...
				if (phouses[*pfront] > 0 && *pi == 0)
					Console::Write("   △:3th player       ▲:4th player        |    {0}'s house| ", owner[*pfront]);
				else if (*pi == 0)
			...
			//印出地名
				...
				Console::Write("|{0,11}元|                                             ", plocaMon[*pback]);
				Console::Write("                                            |{0,11}元| ", plocaMon[*pfront]);
			}
			//印出玩家位置
			else if (*pj == 3) {
				//左邊格子
		...
		//印出房價
			...
			*pback = 24;
			...
			for (*pi = 0; *pi < 9; (*pi)++) {
				...
				Console::Write("|{0,7}元", plocaMon[*pback]);
				(*pback)--;
			}
			Console::Write("|{0,11}元| ", plocaMon[*pback]);
			(*pback)--;
...
void chance(array<int>^ pmoney, array<int>^ pnoTax, int^ pround) {
	...
	Console::WriteLine("   yMMN.               | 機  @@    |              .NMMy");
	Console::WriteLine("   -MMMh               | 會  @@    |              hMMM-");
	Console::WriteLine("    dMMM+              | 卡        |             +MMMd");
...
void community(array<int>^ pmoney, array<int>^ pnoTax, int^ pround) {
	...
	Console::WriteLine("                       |     @@    |");
	Console::WriteLine("   yMN/                |     @@    |               /NMy");
	Console::WriteLine("   yMMN.               | 命  @@    |              .NMMy");
	Console::WriteLine("   -MMMh               | 運  @@    |              hMMM-");
	Console::WriteLine("    dMMM+              | 卡        |             +MMMd");
	Console::WriteLine("    /MMMM-    :mmo`    |     @@    |   `omm:    -MMMM/");
	Console::WriteLine("     dMMMN+`  .hMMNo`   ===========  `oNMMh.  `+NMMMd");
	Console::WriteLine("      .NMMMMNs-  /NMMNo`            `oNMMN/  -sNMMMMN.");
	...
	array<int>^ owner, array<int>^ pposition, int^ people, int^ pround, array<int>^ pmoney, array<int>^ pnoTax, array<String^>^ playerName, String^ showStore)
{
	...
	*pdice = (rand() % 11) + 2;
	...
	//房屋方塊
		...
		//踩上別人家的土地
			...
			if (alive[owner[pposition[*pround]] - 1] == 0) {
				Console::Write("站上了{0}號玩家的土地，需要付{1}元過路費。", owner[pposition[*pround]], (int)plocaMon[pposition[*pround]] * 0.5);
				showStore = showStore + "站上了" + owner[pposition[*pround]] + "號玩家的土地，需要付他" + (int)plocaMon[pposition[*pround]] * 0.5 + "元過路費。\n";
				...
				Console::Write("這塊地主人已死亡，{0}元接管他的土地。", (int)plocaMon[pposition[*pround]] / 2);
				if (pmoney[*pround] > plocaMon[pposition[*pround]] / 2 + 100) {
					pmoney[*pround] -= (plocaMon[pposition[*pround]] / 2); 
					Console::Write("他選擇接管[ {0} ]的房子。 此地點是 {1} 級房屋。", plocations[pposition[*pround]], phouses[pposition[*pround]]);
					showStore = showStore + "接管了" + plocations[pposition[*pround]] + "的房子。 此地點是" + phouses[pposition[*pround]] + "級房屋。\n";
					owner[pposition[*pround]] = (*pround) + 1;
					Console::Write("  (按下ENTER)");
					String^ inputString = Console::ReadLine();
				}
				else {
					Console::Write("他拒絕接管房子，遊戲繼續。");
					showStore = showStore + "拒絕接管房子。\n";
					Console::Write("  (按下ENTER)");
					String^ inputString = Console::ReadLine();
				}
			}
	...
	//為了show存檔
	showStore = showStore + "玩家"+ playerName[*pround] +"擁有" + pmoney[*pround] + "元。\n";
	StreamWriter^ sw = gcnew StreamWriter("ShowEvent.txt", true);
	...
	array<int>^ owner, array<int>^ pposition, int^ people, int^ pround, array<int>^ pmoney, array<int>^ pnoTax, array<String^>^ playerName, String^ showStore)
{
	...
	//房屋方塊
		...
		//踩上別人家的土地
			...
			if (alive[owner[pposition[*pround]] - 1] == 0) {
				Console::Write("你站上了{0}號玩家的土地，需要付他{1}元過路費。(按下ENTER)",owner[pposition[*pround]] , (int)plocaMon[pposition[*pround]]*0.5);
				showStore = showStore + "站上了" + owner[pposition[*pround]] + "號玩家的土地，需要付他" + (int)plocaMon[pposition[*pround]]*0.5 + "元過路費。\n";
				pmoney[*pround] -= (int)plocaMon[pposition[*pround]] * 0.5;
				pmoney[owner[pposition[*pround]]-1] += (int)plocaMon[pposition[*pround]] * 0.5;
				enter = Console::ReadLine();
				...
				Console::Write("擁有這塊地的玩家已經死亡了，這邊是一片充滿怪物的荒地，{0}元可以接管他的土地(y/n): ", (int)plocaMon[pposition[*pround]]/2);
				do {
					String^ YesNo = Console::ReadLine();
					if (String::Compare(YesNo, "y") == 0) {
						pmoney[*pround] -= (plocaMon[pposition[*pround]] / 2);
						Console::Write("接管了[ {0} ]的房子。 此地點是 {1} 級房屋。(按下ENTER)", plocations[pposition[*pround]], phouses[pposition[*pround]]);
						showStore = showStore + "接管了" + plocations[pposition[*pround]] + "的房子。 此地點是" + phouses[pposition[*pround]] + "級房屋。\n";
						owner[pposition[*pround]] = (*pround) + 1;
						enter = Console::ReadLine();
						break;
						...
						Console::Write("拒絕接管房子，遊戲繼續。(按下ENTER)");
						showStore = showStore + "拒絕接管房子。\n";
						enter = Console::ReadLine();
						break;
					}
...
int main(array<System::String ^> ^args)
{
	...
	while (1) {
		...
		//輸入遊戲人數，有做非整數防呆
		do {
			Console::Write("請輸入遊玩人數(2到4人)或輸入load讀取遊戲紀錄檔: ");
			String^ inputString = Console::ReadLine();
			if (String::Compare(inputString, "2") == 0 || String::Compare(inputString, "3") == 0 || String::Compare(inputString, "4") == 0) {
				*people = Convert::ToInt32(inputString);
				...
				do {
					Console::Write("請輸入起始金額(建議1500元，不要輸入負數或是超過一百萬): ");
